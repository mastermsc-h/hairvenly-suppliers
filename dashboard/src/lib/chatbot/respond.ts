...
export async function respondAsBot(sessionId: string, opts: RespondOptions = {}): Promise<RespondResult> {
  ...
  systemPrompt += "- 📏 5CM-TOLERANZ — solution-first formulieren:\n";
  systemPrompt += "  • Wenn die Wunsch-Länge nicht direkt verfügbar ist, aber eine ≤5cm LÄNGERE Variante in derselben Farbe SOFORT da ist → präsentiere diese als PRIMÄRE Lösung. Der Friseur kürzt das beim Einarbeiten problemlos auf die Wunsch-Länge.\n";
  systemPrompt += "  • ❌ FALSCH (apologetisch, dwelling auf Mangel): 'Latte Balayage in 55cm gibt es bei uns leider nicht — die Tapes in dieser Farbe kommen nur in Russisch glatt 60cm.' Diese Formulierung framet 5cm als Hindernis. Eine Kundin hat KEIN Problem mit 5cm länger.\n";
  systemPrompt += "  • ✅ RICHTIG (solution-first): 'Latte Balayage hätten wir in 60cm sofort da [URL] — das ist nur 5cm länger, der Friseur kürzt das beim Einarbeiten auf 55cm. Soll ich dir die vorbereiten? Falls du genau 55cm willst: kommt als Mini Tape ca. 30.05. wieder.'\n";
  systemPrompt += "  • Reihenfolge: (1) verfügbare nahe Variante als Hauptlösung, (2) ETA der exakten Variante als Backup, (3) bei Dringlichkeit andere Farbe sofort verfügbar als zweite Alternative. NICHT umgekehrt.\n";
