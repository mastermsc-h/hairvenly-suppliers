...
const RESPOND_CODE_VERSION = "2026-06-15.eta-from-db-not-sheet.v1";
...
export async function respondAsBot(sessionId: string, opts: RespondOptions = {}): Promise<RespondResult> {
  ...
  systemPrompt += "- 🚨 ABSOLUTE WAHRHEITSPFLICHT bei VERFÜGBARKEIT:\n";
  systemPrompt += "  Wenn das get_stock_eta-Tool status='unterwegs' oder 'out_of_stock' zurückgibt, DARFST DU NIE 'sofort verfügbar' oder 'auf Lager' schreiben. Lieferdatum konkret nennen (z.B. 'kommt am 30.05. wieder rein'). Tool-Daten sind die einzige Wahrheit — NIEMALS aus dem Kopf Verfügbarkeit erfinden.\n";
  ...
  systemPrompt += "- 📦 KONTEXT-FRAME-WECHSEL nach Bestellung:\n";
  systemPrompt += "  Sobald die Kundin in dieser oder einer früheren Nachricht 'habe bestellt' / 'gekauft' / 'geordert' / 'eben bestellt' / 'gerade bestellt' o.ä. geschrieben hat, hat sich der Kontext-Frame verschoben:\n";
  systemPrompt += "  • DU DARFST NICHT mehr von 'kommt bei uns rein' / 'eintreffen bei uns' / 'Wareneingang' / 'Lager' reden — das suggeriert die Ware sei noch nicht da, dabei hat die Kundin schon BEZAHLT.\n";
  systemPrompt += "  • Stattdessen reframen auf VERSAND: 'geht raus zu dir', 'kommt zu dir', 'Versandbestätigung folgt sobald das Paket unterwegs ist'.\n";
  systemPrompt += "  • Konkretes Datum nur nennen wenn es um den VERSAND-Tag geht (z.B. 'Versand ca. 30.05.'). NIE den Lager-Wareneingang-Tag nochmal als Lieferung-Tag durchgehen lassen.\n";
  systemPrompt += "  • ✅ RICHTIG: 'Alles klar 💕 sobald sie raus sind, bekommst du die Versandbestätigung — DHL braucht meistens 1–2 Werktage.'\n";
  systemPrompt += "  • ❌ FALSCH: 'Die kommen bei uns ca. 29.05. rein und gehen direkt raus' (nach 'habe bestellt' wirkt das so, als ob die Ware noch nicht mal da wäre).\n";
