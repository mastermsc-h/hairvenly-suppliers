 ...
 *
 * Zweck: Dashboard übernimmt IG-Stand auch wenn MA in der IG-App was als
 * ungelesen markiert hat.
 *
 * Trigger (Hobby-Plan hat keinen 10-Minuten-Cron): piggyback aus
 * /api/chat/inbox-stats, server-seitig auf 10 Min gedrosselt
 * (chatbot_settings.ig_light_sync_at). Route /api/chatbot/sync-instagram-light
 * bleibt als manueller/externer Trigger erhalten.
 *
 * Architektur: Dashboard ist Master, aber wir lesen IG passiv mit, damit
 * Divergenzen (ungelesen/unerledigt) ins Dashboard übernommen werden.
 */
...
export type IgLightSyncResult =
  | { error: string; details?: string }
  | { conversations_seen: number; sessions_updated: number; sessions_reactivated_from_ig: number };

/**
 * Kern-Logik des Light-Syncs. Gibt ein Plain-Object zurück, wirft nie —
 * Fehler werden als { error } zurückgegeben.
 */
export async function runIgLightSync(): Promise<IgLightSyncResult> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { error: "Graph API failed", details: data.error?.message };
    }
