...
export default async function PrepaymentsPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-neutral-900">{t(locale, "finance.prepayments.title")}</h1>
        <p className="text-sm text-neutral-500 mt-1">{t(locale, "finance.prepayments.subtitle")}</p>
      </div>
      <div className="bg-white rounded-2xl border border-neutral-200 p-8 shadow-sm flex flex-col items-center justify-center gap-4 text-center min-h-[300px]">
        ...
        <p className="text-neutral-500">{t(locale, "finance.prepayments.coming_soon")}</p>
      </div>
    </div>
  );
}
