    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 space-y-4">
          ...
          {localAddress && (() => {
            const incomplete = addressLooksIncomplete(localAddress.address1);
            return (
              <div className={`rounded-2xl border-2 p-4 shadow-sm text-sm ${incomplete ? "bg-amber-50 border-amber-400" : "bg-white border-neutral-200"}`}>
                ...
                {incomplete && (
                  <div className="mb-2 p-2 rounded-lg bg-amber-100 border border-amber-300 text-xs text-amber-900 font-medium flex items-start gap-2">
                    <AlertTriangle size={14} className="shrink-0 mt-0.5" />
                    <div>
                      <strong>Adresse unvollständig — Hausnummer fehlt!</strong>
                      <div className="font-normal mt-0.5">DHL akzeptiert keine Adresse ohne Hausnummer.</div>
                    </div>
                  </div>
                )}
                <div className="text-neutral-700 leading-relaxed">
                  {localAddress.name}<br />
                  {localAddress.address1}<br />
                  {localAddress.address2 && (<>{localAddress.address2}<br /></>)}
                  {localAddress.zip} {localAddress.city}<br />
      ...
      {/* Adress-Edit-Modal */}
      {addressModalOpen && localAddress && (
        <AddressEditModal
          sessionId={sessionId}
          initial={localAddress}
          onClose={() => setAddressModalOpen(false)}
          onSaved={(updated) => {
            setLocalAddress({ ...localAddress, ...updated });
...
function AddressEditModal({
  ...
  sessionId: string;
  initial: { name: string | null; address1: string | null; address2: string | null; zip: string | null; city: string | null; country: string | null };
  onClose: () => void;
  onSaved: (updated: { address1: string; address2: string; zip: string; city: string; country: string }) => void;
}) {
  ...
  function handleSave() {
    ...
    startSavingTransition(async () => {
      const res = await updateShippingAddress(sessionId, {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3 mb-4">
          <div>
            <div className="text-lg font-bold text-neutral-900">Lieferadresse bearbeiten</div>
            <div className="text-sm text-neutral-600 mt-1">
              Wird direkt in Shopify aktualisiert. Hausnummer in der Straßenzeile mit angeben (z.B. „Hauptstr. 5b").
            </div>
          </div>
          <button onClick={onClose} className="text-neutral-400 hover:text-neutral-700 shrink-0" aria-label="Schließen">
            <X size={20} />
        ...
        <div className="space-y-3">
          <div>
            ...
            <input
              ...
              className={`w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 ${
                addressLooksIncomplete(address1)
                  ? "border-amber-400 focus:ring-amber-500"
                  : "border-neutral-300 focus:ring-neutral-900"
              }`}
              placeholder="z.B. Hauptstr. 5b"
            />
            {addressLooksIncomplete(address1) && (
              <div className="text-xs text-amber-700 mt-1">⚠ Hausnummer fehlt!</div>
            )}
          </div>
          <div>
            <label className="text-xs font-medium text-neutral-600 uppercase tracking-wide block mb-1">
              Adresszusatz <span className="font-normal text-neutral-400">(optional)</span>
            </label>
            <input
              type="text"
              value={address2}
              ...
              placeholder="z.B. Hinterhaus, c/o, Etage"
            />
          </div>
          <div className="grid grid-cols-3 gap-2">
            <div>
              <label className="text-xs font-medium text-neutral-600 uppercase tracking-wide block mb-1">PLZ</label>
              <input
                type="text"
                value={zip}
            ...
            <div className="col-span-2">
              <label className="text-xs font-medium text-neutral-600 uppercase tracking-wide block mb-1">Ort</label>
              <input
                type="text"
                value={city}
          ...
          <div>
            <label className="text-xs font-medium text-neutral-600 uppercase tracking-wide block mb-1">Land</label>
            <input
              type="text"
              value={country}
          ...
          {error && (
            <div className="p-2 rounded-lg bg-red-50 border border-red-300 text-xs text-red-800">{error}</div>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-2 mt-5">
          ...
          <button
            type="button"
            onClick={handleSave}
            disabled={pending || !address1.trim() || !zip.trim() || !city.trim()}
            className="px-4 py-2 rounded-lg bg-neutral-900 text-white text-sm font-medium hover:bg-neutral-700 transition disabled:opacity-40"
          >
            {pending ? "Speichern…" : "In Shopify speichern"}
          </button>
        </div>
      </div>
    </div>
