...
| 2026-05 | **Sheet-vs-Shopify-Divergenz: Bot sagt „auf Lager", Shopify zeigt „Ausverkauft"** — Stock Calculation Sheet zählt physischen Lagerbestand in Bremen, Shopify zeigt sellable inventory (− Reservierungen − Pending Orders). User-Bug: Kundin auf Produktseite hairvenly.de/products/4-27t24-tape-extensions-65cm sah „Ausverkauft", Bot sagte „aktuell auf Lager + nächste Lieferung 25.06" → Vertrauensbruch | **Shopify-Live-Check** via öffentliches `/products/<handle>.js` Endpoint (kein Auth, `data.available: true/false`). Quelle der URL: `InventoryRow.url` (direkt aus Sheet-Spalte F), Fallback `urlMap` aus `product_colors`. 5min In-Memory-Cache pro Vercel-Function-Instance, 1.5s Timeout, Bulk-Parallel via `Promise.all`. Bei `available=false` wird die Row aus `withStock` rausgefiltert UND eine synthetische Nullbestand-Eintrag erzeugt → Bot sieht "out_of_stock" oder fällt auf ETA aus `inUnterwegs` zurück. Bei null (Network-Fail/Timeout/keine URL): trust-the-Sheet-Fallback, alte Logik unverändert. Verifiziert: 4/27T24 → ausverkauft (876ms), 3T8A → available (109ms), unbekanntes Handle → null (100ms) | `shopify-availability.ts` (neu), `tools/index.ts` get_stock_eta |
