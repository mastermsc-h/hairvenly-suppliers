...
| 2026-05 | **Lange Wartezeit → Bot soll Nachbar-Länge in gleicher Farbe als Alternative anbieten** (User-Wunsch 2026-05-27: „4/27T24 65cm braucht 4 Wochen → 55cm in derselben Farbe ist sofort verfügbar, also empfehlen") | **Nearby-Length-Alternative-Lookup** in `get_stock_eta`. Wenn search EXPLIZIT eine Länge enthält: parallele Re-Suche im inventoryRows mit `tokensWithoutLength` (Längen-Token entfernt), gleiche Farbe/Methode/Linie. Filter qty>0, sortiert nach `Math.abs(lenCm - requestedCm)`, max 20cm Differenz. Trigger-Bedingungen: (a) Pfad unterwegs UND ETA > 14 Tage, (b) Pfad out_of_stock_no_eta (Nullbestand, kein Datum), (c) Pfad out_of_stock (qty=0 ohne Nachschub). In allen 3 Fällen: `nearby_alternative` Feld + Bot-Instruktion mit konkreter Phrasierung („wir hätten sie in [X]cm sofort verfügbar, nur [Y]cm Unterschied"). Compound-Color-Guard wird auch hier angewandt → kein Cross-Color-Alternative. | `tools/index.ts` getStockEta |
